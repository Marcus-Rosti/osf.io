...
def update_analytics(node, file_id, version_idx, view_flag):
    """
    ...
    :param bool view_flag: is the file being viewed or downloaded
    """
    if(view_flag):
        update_counter(u'view:{0}:{1}'.format(node._id, file_id))
        update_counter(u'view:{0}:{1}:{2}'.format(node._id, file_id, version_idx))
    else:
